...
def forward(one, two, three, mult):
	...
	if(one.lower() == "f"):
		...
		command.angular.y = (float(three)* -1.0) + 2
		command.linear.x = 0.0
		command.linear.y = 0.0
	elif(one.lower() == "l"):
		command.angular.z = float(two)
		command.angular.y = float(three) - 2
		command.linear.x = 0.0
		command.linear.y = 0.0
	print("published command:" + str(command))
	if(command.angular.y != 0):
		command.angular.x == 1
	pub.publish(command)
	if not mult:
		rospy.sleep(4.0)
...
if __name__== '__main__':
	...
	while(mode != 3):
		...
		if(mode == 1):
			...
			for j in c:
				commands = j.split()
				forward(commands[0], commands[1], commands[2], True)
		elif(mode != 3):
